...
pub struct EventLoop<T: 'static> {
  ...
  #[cfg(feature = "x11")]
  device_thread: Option<device::DeviceEventThread>,
}
...
impl<T: 'static> EventLoop<T> {
  ...
  fn new_gtk(app_id: Option<&str>) -> Result<EventLoop<T>, Box<dyn Error>> {
    ...
    #[cfg(feature = "x11")]
    let (run_device_thread, device_thread) = if window_target.is_x11() {
      ...
      let device_thread = device::spawn(device_tx);
      ...
      (Some(run_device_thread), Some(device_thread))
    } else {
      (None, None)
    };
    #[cfg(not(feature = "x11"))]
    ...
    F: FnMut(Event<'_, T>, &RootELW<T>, &mut ControlFlow),
  {
    ...
    let exit_code = context
      ...
      .unwrap_or(1);
    // Stop and join before callers can exit the process and run native library
    // finalizers. Dropping the GLib receiver alone does not wake XNextEvent.
    #[cfg(feature = "x11")]
    drop(self.device_thread.take());
    exit_code
  }
